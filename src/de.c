...
// minimal layout of DE device reigsters to init & configure mixer/blender/ui
// layer
typedef union {
    struct {
        uint32_t sclk_gate;
...
typedef union {
    struct {
        ...
        // not sure re: # of pipes? (D1 manual says 4 in one place and 2 in
        // another) we are currently only using pipe [1]
...
_Static_assert(&(DE_BASE->regs.de2tcon_mux) == (uint32_t *) 0x5000010,
               "de de2tcon_mux reg must be at address 0x5000010");
_Static_assert(&(DE_MIXER0->regs.glb_dbuffer) == (uint32_t *) 0x5100008,
               "de mixer0 glb_dbuffer reg must be at address 0x5100008");
_Static_assert(&(DE_BLD0->regs.pipe[1].offset) == (uint32_t *) 0x510101c,
               "de blender0 pipe[1] offset reg must be at address 0x510101c");
_Static_assert(&(DE_UI_CH1->regs.top_haddr) == (uint32_t *) 0x5103080,
               "de ui ch1 topaddr reg must be at address 0x5103080");
_Static_assert(&(DE_SCALER->regs.horiz_step) == (uint32_t *) 0x5140088,
               "de scaler horiz step reg must be at address 0x5140088");
...
void de_init(int fb_width, int fb_height, int screen_width, int screen_height) {
    ...
    module.de->regs.ahb_reset = module.de->regs.sclk_gate =
        module.de->regs.hclk_gate = 1; // 1 to ungate mixer0

    // de_size_t registers are slightly wacky: actual width/height = (stored
    // value + 1)
    de_size_t full_screen = {.width = screen_width - 1,
...
// (optional scaler) -> blender -> output to TCON Mixer-0 more full featured (1
// video channel, 3 UI overlay) Mixer-1 only has 1 video + 1 UI
static void de_config_mixer0(de_size_t full_screen) {
    module.de_mixer0->regs.glb_ctl = 1; // enable mixer 0
    module.de_mixer0->regs.glb_size = full_screen;
}

// DE Blender pairwise composites 2 overlay channels together. Three separate
// blenders allow blending 4 channels. We use only blender 0 with single UI
// channel.
static void de_config_blender0(de_size_t full_screen) {
    ...
    module.de_bld0->regs.pipe_ctrl =
        ((1 << pipe_index) << 8); // enable pipe index 1
    module.de_bld0->regs.pipe[pipe_index].input_size = full_screen;
    module.de_bld0->regs.route = 0x3210; // channels 0-3, each channel routed to
                                         // pipe at corresponding index
}
...
// DE UI Overlay represents a single framebuffer. Mixer0 has three UI overlay
// channels, we use only channel 1. An optional UI scaler can be used to
// up/downscale from framebuffer input on route to blender.
static void de_config_ui_ch1(de_size_t fb_size, de_size_t full_screen) {
    // default alpha @[24], top-addr-only @[23] no premul @[16] format @[8]
    // enable fill @[4] use global alpha @[1] enable @[0]
    uint32_t features = (0xff << 24) | (0 << 23) | (0 << 16) |
                        (XRGB_8888 << 8) | (1 << 4) | (1 << 1) | (1 << 0);
    module.de_ui_ch1->regs.layer[0].attr_ctrl = features;
    module.de_ui_ch1->regs.layer[0].size = fb_size;
    module.de_ui_ch1->regs.layer[0].offset =
        0; // position @ top left corner of output
    module.de_ui_ch1->regs.layer[0].pitch_nbytes =
        (fb_size.width + 1) * 4; // 4 bytes (32 bits) per pixel
    ...
    de_config_ui_scaler(
        fb_size,
        full_screen); // will center on screen and apply scaler if necessary
}
...
static int scale_factor(int in, int out) {
    return ((in * 32) + out - 1) /
           out; // round up to nearest 32nd (minimum scale for DE2)
}

static int compute_scale_step(de_size_t fb_size, de_size_t full_screen,
                              de_size_t *p_scaled_size,
    ...
    // JDZ: for now, only apply scaling if at least 2x (scale to partial pixels
    // has poor result) otherwise do not scale, center orig rect on screen as-is
    ...
    *p_scaled_size =
        (de_size_t){.width = output_width - 1, .height = output_height - 1};
    ...
    *p_offset = (margin_y << 16) | margin_x; // position in center
    return scale_f << 15; // scale factor stored as x.15 fixed point (only 5
                          // bits of fraction used tho')
}
...
static void de_config_ui_scaler(de_size_t fb_size, de_size_t full_screen) {
    ...
    int step = compute_scale_step(fb_size, full_screen, &scaler_output_size,
                                  &center_offset);
    module.de_bld0->regs.pipe[1].offset = center_offset; // position in center
    if (step == 0x100000) {
        module.de_scaler->regs.ctrl = 0; // disable scaler
        module.de_bld0->regs.pipe[1].input_size =
            fb_size; //  ui layer is direct input to blender pipe 1
    } else {
        module.de_scaler->regs.ctrl = 1;              // enable scaler
        module.de_scaler->regs.horiz_phase = 1 << 20; // correct for first pixel
        module.de_scaler->regs.horiz_step = module.de_scaler->regs.vert_step =
            step;
        module.de_scaler->regs.input_size =
            fb_size; // ui layer is input to scaler
        module.de_scaler->regs.output_size = scaler_output_size;
        module.de_bld0->regs.pipe[1].input_size =
            scaler_output_size; // scaler output is input to blender pipe 1

        // UI scaler works line by line, array of coeff controls of 4-tap blend
        // within line (horiz) below I am setting coeffs to 0,0,0,64 to
        // replicate right pixel of each 4-tap (hard cutoff) note that vert is
        // fixed linear scale of taps without configurable control (fuzzy
        // instead of crisp...) if this ends being unacceptable, could switch to
        // video scaler which has controls for both horiz and vert
        for (int i = 0; i < 16; i++)
            module.de_scaler->regs.horiz_coeff[i] = 0x40;
